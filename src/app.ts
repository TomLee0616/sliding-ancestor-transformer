...
// 键盘步进配置：键名 → 等级增量。
const KEY_STEPS: Record<string, number | "home" | "end"> = {
  ArrowLeft: 0.5,
  ArrowRight: -0.5,
  ArrowUp: 1,
  ArrowDown: -1,
...
/** 把整套 UI 挂载到 root，并返回控制器。 */
export function mountApp(
  root: HTMLElement,
  onLevelChange: LevelChangeHandler = () => undefined,
): AppController {
  root.innerHTML = `
    <div class="experience" data-stage="0" tabindex="0">
      <header class="masthead">
        <div>
          <p class="eyebrow">NING INTENSITY CALIBRATOR</p>
          <h1>滑动变祖器</h1>
        </div>
      ...
      <section class="portrait-zone" aria-labelledby="current-stage-label">
        <p class="stage-ghost" aria-hidden="true">小难宁</p>
        <div class="portrait-shell">
          <div class="imperial-halo" aria-hidden="true"></div>
          <canvas class="portrait-canvas" role="img" aria-label="当前形态：小难宁"></canvas>
        ...
        <div class="stage-readout">
          ...
          <p class="stage-name" aria-live="polite">小难宁</p>
          <span class="stage-index">阶段 01 / 06</span>
        </div>
      </section>

      <section class="control-panel" aria-label="宁系强度控制">
        <div class="range-wrap">
          ...
          <input
            ...
            value="0"
            aria-label="宁系强度"
            aria-valuetext="小难宁，0 级，共 30 级"
            disabled
          />
        </div>
        <ol class="stage-markers">${createStageMarkers()}</ol>
        <p class="drag-hint"><span aria-hidden="true">←</span> 拖动或用方向键增强宁系浓度 <span aria-hidden="true">→</span></p>
      </section>
  ...
  // 键盘增强：原生 range 已支持方向键，这里接管 .experience 上的按键，
  // 让用户无需精确聚焦滑块也能操作，并提供 PageUp/Down 跨阶段跳转。
  experience.addEventListener("keydown", (event: KeyboardEvent) => {
    const action = KEY_STEPS[event.key];
    if (action === undefined) {
      return;
    }
    ...
    if (action === "home") {
      ...
      setLevel(currentPosition + action);
    }
  });
